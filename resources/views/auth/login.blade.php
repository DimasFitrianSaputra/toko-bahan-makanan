<x-guest-layout>
    <div class="mb-8 text-center">
        <h1 class="text-4xl font-bold text-white">Masuk ke Toko Bahan Makanan</h1>
        <p class="mt-2 text-sm text-slate-400">Gunakan dashboard kasir modern untuk mengelola transaksi toko.</p>
    </div>

    <x-auth-session-status class="mb-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm text-emerald-100" :status="session('status')" />
    ...
    <form x-data="{ showPassword: false, submitting: false }" @submit="submitting = true" method="POST" action="{{ route('login') }}" class="space-y-6">
        @csrf
        ...
        <div class="flex items-center justify-between gap-3">
            <label for="remember_me" class="inline-flex items-center gap-2 text-sm text-slate-300">
                <input id="remember_me" type="checkbox" name="remember" class="h-4 w-4 rounded border-slate-700 bg-slate-950 text-emerald-500 focus:ring-emerald-500" />
                <span>Remember me</span>
            </label>

            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-slate-400 hover:text-white transition">Lupa password?</a>
            @endif
        </div>
    ...
    <p class="mt-8 text-center text-sm text-slate-400">
        Belum punya akun? <a href="{{ route('register') }}" class="font-semibold text-white hover:text-emerald-300 transition">Daftar sekarang</a>
    </p>
</x-guest-layout>
